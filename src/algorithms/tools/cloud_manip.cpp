...
std::vector<point_xy_rgb> cloud_to_2d_rgb(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_ptr)
{
    ...
    for (unsigned int cloud_it = 0; cloud_it < cloud_ptr->points.size(); cloud_it++)
    {
        ...
        pt_xy_rgb.x = cloud_ptr->points[cloud_it].x;
        pt_xy_rgb.y = cloud_ptr->points[cloud_it].y;
        pt_xy_rgb.r(cloud_ptr->points[cloud_it].r);
        pt_xy_rgb.g(cloud_ptr->points[cloud_it].g);
        pt_xy_rgb.b(cloud_ptr->points[cloud_it].b);
