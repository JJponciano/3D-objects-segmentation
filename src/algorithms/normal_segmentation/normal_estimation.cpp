...
void normal_estimation(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, float radius, int max_neighbs)
{
    ...
    for (cloud_it = 0; cloud_it < cloud->points.size(); cloud_it++)
    {
        // if there are neighbours left
        if (kdt.radiusSearch(cloud->points[cloud_it], radius, pointIdxRadiusSearch,
                             pointRadiusSquaredDistance, max_neighbs) > 0)
        {

            for (unsigned int pt_index = 0; pt_index < (pointIdxRadiusSearch.size() - 1); pt_index++)
            {
                vect_1 = geom::vectors::vect_2pts((cloud->points[cloud_it]),
                                                 cloud->points[pointIdxRadiusSearch[pt_index + 1]]);

                // defining the second vect_or; making sure there is no 'out of bounds' error
                if (pt_index == pointIdxRadiusSearch.size() - 2)
                    vect_2 = geom::vectors::vect_2pts((cloud->points[cloud_it]),
                                                      cloud->points[pointIdxRadiusSearch[1]]);


                else
                    vect_2 = geom::vectors::vect_2pts((cloud->points[cloud_it]),
                                                      cloud->points[pointIdxRadiusSearch[pt_index + 2]]);
            ...
            geom::aux::normal_to_rgb(&(cloud->points[cloud_it]), normal);
